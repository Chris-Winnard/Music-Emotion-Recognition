...
from torcheeg.datasets.constants.utils import format_region_channel_list
...
MYDATA_CHANNEL_LIST = ['Fp1', 'Fpz', 'Fp2', 'F7', 'F3', 'Fz', 'F4', 'F8', 'FC5', 'FC1', 'FC2',
                       'FC6', 'M1', 'T7', 'C3', 'Cz', 'C4', 'T8', 'M2', 'CP5', 'CP1', 'CP2',
                       'CP6', 'P7', 'P3', 'Pz', 'P4', 'P8', 'POz', 'O1', 'Oz', 'O2']

MYDATA_GENERAL_REGION_LIST = format_region_channel_list(MYDATA_CHANNEL_LIST, GENERAL_REGION_LIST)
                ...
                for i, (train_test_dataset, val_dataset) in enumerate(split.split(dataset)):
                    for j, (train_val_dataset, test_dataset) in enumerate(split.split(dataset)):
                        if j == i+1 or (i == n_splits - 1 and j == 0):  #Rotating such that when Fold 0 is used for validation, fold 1 is used for test, ... fold 4 used for val, fold 0 used for test
                            ...
                            overlapping_clip_ids = set(train_val_clip_ids).intersection(train_test_clip_ids) #Will order matter?
                            ...
                            #Define your model
                            model = LGGNet(in_channels=1,region_list=MYDATA_GENERAL_REGION_LIST, chunk_size=203, num_electrodes=32, hid_channels=32, num_classes=2)
